...
function GradText({ children }) {
  return <span style={{ background: 'var(--grad-text)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{children}</span>
}
...
export default function ProposalView({ id, onBack }) {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', color: 'var(--fg40)', fontFamily: 'var(--mono)', fontSize: 12 }}>
      Carregando proposta...
    </div>
  )
  ...
  if (!proposal) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--fg)' }}>Proposta não encontrada.</div>
  ...
  const tryUnlock = () => {
    ...
    if (clean(phone) === clean(proposal.client_phone)) {
      setUnlocked(true)
  ...
  const handleVerdict = async (v) => {
    await updateProposalStatus(id, v)
    setVerdict(v)
  }
  ...
  if (!unlocked) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', padding: 20 }}>
      <div style={{ background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 'var(--r16)', padding: 48, width: '100%', maxWidth: 400, textAlign: 'center' }}>
        ...
        <div style={{ fontFamily: 'var(--sans)', fontSize: 22, fontWeight: 800, color: 'var(--fg)', marginBottom: 6, letterSpacing: -1 }}>Acesso à Proposta</div>
        <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--fg40)', marginBottom: 28, letterSpacing: .5 }}>Digite seu número de celular para acessar</div>
        {err && <div style={{ background: 'rgba(239,68,68,.08)', border: '1px solid rgba(239,68,68,.25)', borderRadius: 'var(--r8)', padding: '9px 13px', fontSize: 12, color: '#ef4444', marginBottom: 14 }}>{err}</div>}
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', padding: 20 }}>
      <div style={{ maxWidth: 480, textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 15, color: 'var(--fg40)', lineHeight: 1.8, marginBottom: 28 }}>
          {verdict === 'accepted'
            ? 'Ótimo! Entraremos em contato em breve para dar início ao projeto.'
            : 'Obrigado pelo retorno. Se mudar de ideia, é só nos chamar.'}
        </div>
        <a href={`https://wa.me/5511941543335?text=Olá Ednilson! ${verdict === 'accepted' ? 'Acabei de aceitar a proposta' : 'Vi a proposta'} "${proposal.headline || ''}".`}
          target="_blank" rel="noopener noreferrer" className="btn-grad" style={{ justifyContent: 'center', textDecoration: 'none' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: 'var(--sans)' }}>
      ...
      <div style={{ position: 'sticky', top: 0, background: 'rgba(13,13,13,.96)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--border)', padding: '13px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 50 }}>
        <div style={{ fontFamily: 'var(--sans)', fontSize: 18, fontWeight: 900, color: 'var(--fg)', letterSpacing: -1 }}><GradText>hi</GradText>hat</div>
        <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--fg40)' }}>Proposta para {p.client_name}</div>
        {onBack && <button className="btn-ghost" onClick={onBack}>← Voltar</button>}
      </div>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '56px 32px 100px' }}>
        ...
        <div style={{ marginBottom: 56, paddingBottom: 56, borderBottom: '1px solid var(--border)' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--a2)', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 12 }}>hihat agency · Proposta Comercial</div>
          <h1 style={{ fontFamily: 'var(--sans)', fontSize: 'clamp(32px, 6vw, 72px)', fontWeight: 900, letterSpacing: -3, lineHeight: .92, color: 'var(--fg)', marginBottom: 16 }}>{p.headline || 'Proposta Comercial'}</h1>
          {p.subheadline && <div style={{ fontSize: 18, color: 'var(--fg40)', marginBottom: 20 }}>{p.subheadline}</div>}
          {(p.badges || []).length > 0 && (
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
              {p.badges.map((b, i) => (
                <span key={i} style={{ fontFamily: 'var(--mono)', fontSize: 10, padding: '5px 14px', border: '1px solid rgba(255,107,0,.3)', borderRadius: 'var(--r100)', color: 'var(--a2)', background: 'rgba(255,107,0,.08)' }}>{b}</span>
              ))}
            </div>
          )}
          <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--fg40)' }}>Válida por {p.validity || 7} dias · Emitida em {p.created_at}</div>
        </div>
              ...
              {p.services.map((svc, i) => (
                <div key={i} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--r16)', padding: 28 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                    <span style={{ fontFamily: 'var(--mono)', fontSize: 10, background: 'rgba(255,107,0,.1)', border: '1px solid rgba(255,107,0,.25)', color: 'var(--a2)', padding: '3px 10px', borderRadius: 'var(--r4)' }}>0{i + 1}</span>
                    <div style={{ fontSize: 17, fontWeight: 800, color: 'var(--fg)' }}>{svc.label}</div>
                  </div>
                  {svc.deliverables && <p style={{ fontSize: 14, color: 'var(--fg40)', lineHeight: 1.8, marginBottom: 16 }}>{svc.deliverables}</p>}
                    ...
                    <div>
                      ...
                      {svc.includes.filter(Boolean).map((inc, j) => (
                        <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                          <span style={{ color: 'var(--a2)', fontSize: 12 }}>→</span>
          ...
          <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--r16)', overflow: 'hidden', marginBottom: 16 }}>
            ...
            {(p.items || []).map((it, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 60px 120px', padding: '14px 20px', borderBottom: '1px solid var(--border)', alignItems: 'center' }}>
                ...
                <div style={{ textAlign: 'right', fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--fg)' }}>{fmt((parseFloat(it.price) || 0) * (parseInt(it.qty) || 1))}</div>
              </div>
            ))}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '20px', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: 30, fontWeight: 900, background: 'var(--grad-text)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{fmt(p.total || 0)}</span>
            </div>
          </div>
          {(p.payment || []).length > 0 && (
        ...
        <div style={{ background: 'var(--bg3)', border: '1px solid rgba(255,107,0,.2)', borderRadius: 'var(--r16)', padding: 40, textAlign: 'center' }}>
          <div style={{ fontSize: 22, fontWeight: 900, color: 'var(--fg)', marginBottom: 8, letterSpacing: -1 }}>O que você decide?</div>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button className="btn-grad" style={{ justifyContent: 'center', padding: '15px 36px', fontSize: 15 }} onClick={() => handleVerdict('accepted')}>
              <Icon n="ok" s={16} /> Aceitar proposta
            </button>
            <a href={`https://wa.me/5511941543335?text=Oi Ednilson! Tenho uma dúvida sobre a proposta "${p.headline || ''}".`}
              target="_blank" rel="noopener noreferrer" className="btn-outline" style={{ textDecoration: 'none', justifyContent: 'center', padding: '14px 28px' }}>
              <Icon n="wa" s={15} /> Tirar dúvida
            </a>
            <button className="btn-danger" style={{ padding: '14px 22px' }} onClick={() => handleVerdict('declined')}>
