...
export default function AdminPanel() {
  ...
  const [proposals, setProposals] = useState(() => Object.values(getProposals()).sort((a, b) => b.id > a.id ? 1 : -1))
  ...
  const refresh = () => {
    setProposals(Object.values(getProposals()).sort((a, b) => b.id > a.id ? 1 : -1))
  }
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
              <div>
                ...
                <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--fg40)', marginTop: 3 }}>
            ...
            <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--r12)', overflow: 'hidden' }}>
              ...
              {proposals.length === 0 ? (
                ...
                >
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--fg)' }}>{p.clientName}</div>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--fg40)', marginTop: 1 }}>{p.clientPhone}</div>
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--fg40)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 200 }}>
                    {p.headline || '—'}
                  </div>
                  <div>
                    <span className={`badge ${statusClass[p.status] || 'badge-active'}`}>
                      {statusLabel[p.status] || 'Ativa'}
                    </span>
                  </div>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--fg40)' }}>{p.createdAt}</div>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, background: 'var(--grad-text)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                    {fmt(p.total || 0)}
                  </div>
        ...
        {tab === 'leads' && (
          <>
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--fg40)', marginTop: 3 }}>
                {leads.length} contato{leads.length !== 1 ? 's' : ''} capturado{leads.length !== 1 ? 's' : ''}
              </div>
            </div>
            <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--r12)', overflow: 'hidden' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 120px', padding: '11px 18px', borderBottom: '1px solid var(--border)', fontFamily: 'var(--mono)', fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, color: 'var(--fg40)' }}>
                <span>Nome</span><span>E-mail</span><span>Data</span>
              </div>
              {leads.length === 0 ? (
                <div style={{ padding: '32px 18px', fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--fg40)' }}>Nenhum lead ainda.</div>
              ) : leads.map((l, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 120px', padding: '13px 18px', borderBottom: '1px solid rgba(255,255,255,.04)', fontSize: 13, color: 'var(--fg)' }}>
                  <span>{l.nome || '—'}</span>
                  <span style={{ color: 'var(--fg40)' }}>{l.email}</span>
                  <span style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--fg40)' }}>{l.createdAt}</span>
                </div>
              ))}
            </div>
          </>
